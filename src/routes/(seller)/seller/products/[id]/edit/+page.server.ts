...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	if (product.seller_id !== locals.session.user.id) {
		throw error(403, 'You do not have permission to edit this product');
	}
	...
	const { data: metricValues } = await supabase
		...
		.select(`
			...
			string_value,
			boolean_value,
			metric_definitions (code)
		`)
		.eq('product_id', productId);

	// Convert metrics to a map for easy access
	const metrics: Record<string, any> = {};
	metricValues?.forEach((mv: any) => {
		const code = mv.metric_definitions?.code;
		if (code) {
			metrics[code] = mv.numeric_value ?? mv.string_value ?? mv.boolean_value;
		}
	});
...
export const actions: Actions = {
	default: async ({ request, locals, params }) => {
		...
		try {
			...
			let logo_url = existingProduct.logo_url;
			if (logoFile && logoFile.size > 0) {
			...
			let demo_visual_url = existingProduct.demo_visual_url;
			if (demoVisualFile && demoVisualFile.size > 0) {
			...
			// Then insert new metrics if provided
			const metricInserts = [];
